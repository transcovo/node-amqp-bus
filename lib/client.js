...
function* createBusClient(url, options) {
  ...
  const busClient = Object.create(EventEmitter.prototype);
  busClient.channel = channel;
  busClient.connection = connection;
  busClient.setupQueue = setupQueue;
  busClient.consume = consume;
  busClient.listen = listen;
  busClient.publish = publish;
  busClient.close = function* close() {
  ...
  function* consume(queue, handler) {
    yield channel.consume(queue, co.wrap(function* _consumeMessage(message) {
      ...
      try {
        ...
        busClient.emit('error', new Error('Content is not a valid JSON'), { err, queue, message });
      ...
      try {
        ...
        busClient.emit('error', new Error('Consumer handler failed'), { err, queue, message });
